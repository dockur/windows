...
| `USERNAME` | `Docker` | Name of the local Windows account. Used as the domain account and required when `DOMAIN` is set. |
| `PASSWORD` | `admin` | Password for the local Windows account. Used as the domain password and required when `DOMAIN` is set. |
| `DOMAIN` |  | Active Directory domain to join during installation. Supported for Windows 10 and 11. |
